	...
	\section{Model}
		\subsection{\# AudioLib}
			\begin{itemize}
				...
				\change[AddTrack(...)]{Hat nun \code{string storage} als weiteren Parameter.}
			\end{itemize}
		...
		\subsection{+ ColorManager}
			...
			\paragraph{Attribute}
				\begin{itemize}
					\add[public List$<$CustomColor$>$ Colors]{Alle verfügbaren Farben}
					\add[public CustomColor CurrentColor]{Aktuelle Farbe}
					\add[public void ResetColors()]{Setzt Farb Thema zurück}
				\end{itemize}
		\subsection{+ CustomColor}
			Wrapper Klasse um eine Xamarin Color.
			\begin{itemize}
				\add[public Color Color]{Die Grund Farbe}
				\add[public string Name]{Name der Farbe}
			\end{itemize}
	...
	\section{Model}
		...
		\subsection{\# AudioPlayer}
			\begin{itemize}
				...
				\add{ChangeToSpotifyPlayer(): Wechselt zum Spotify Modus}
				\add{ChangeToBasicPlayer(): Wechselt zum Standart Modus}
				\add{Clear(): Clears Queue and PlayedSongs Stack}
		...
		\subsection{\# SettingsHandler}
			...
			\begin{itemize}
				\add{event LangChanged: Sprache ändert sich}
				\add{event DeviceNameChanged: GeräteName der Earables \change}
				\add{event StepsChanged: Schritt-Anzahl \change}
				\add{event ColorChanged: Farb-Thema \change}
				\add{event ChartChanged: Anzeige-Graph \change}
